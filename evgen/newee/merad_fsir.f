...
!===============================================================================
      subroutine merad_genvt1z(t, vmin, vmax, vgen, t1gen, zgen)
! t: Mandelstam variable t (MeV^2)
! vgen: generated elasticity of missing mass v (MeV^2)
! t1gen, zgen: generated t1 and z
     &bind(C, name = "merad_genvt1z")
      use, intrinsic :: ISO_C_BINDING
      implicit none
      real(C_DOUBLE), intent(IN), VALUE :: t, vmin, vmax
      real(C_DOUBLE), intent(OUT) :: vgen, t1gen, zgen
      real*8 fsir, urand
      real*8 u, sig, sigrand
      real*8 vvb, vva, sib, sia
      real*8 t1min, t1max, tt1b, tt1a, t1z, t1z1, t1z2, t1p
      real*8 zmin, zmax, zzb, zza, det
      integer iv, i, it1, idx, iz
      integer nn
      dimension t1p(5)
      include 'merad_const.inc'
      ...
      do iv = 1, nv
         vva = vvb
         sia = sib
         vvb = vmin + (vmax - vmin) * grv(iv)
         sib = fsir(t, 0d0, vvb, 0d0, 0d0, nn, 2, 0d0)
         if (iv .eq. 1) then
            distsiv(iv) = 0d0
         else
     ...
     .           (sib + sia) * (vvb - vva) / 2d0
      ...
      sigrand = urand() * sig
      do iv = 2, nv
         if (distsiv(iv) .gt. sigrand) then
            vgen = distarv(iv - 1) + (distarv(iv) - distarv(iv - 1)) *
     .           (sigrand - distsiv(iv - 1)) /
     .           (distsiv(iv) - distsiv(iv - 1))
            exit
         endif
      enddo
      ...
      t1min = (2d0 * m2 * t + vgen *
     .     (t - vgen - sqrt((t - vgen)**2 - 4d0 * m2 * t))) /
     .     (2d0 * (m2 + vgen))
      t1max = m2 * t**2 / (m2 + vgen) / t1min
      t1z = (-s * t * (t + u) + 2d0 * m2 * vgen**2) /
     .     ((s - vgen)**2 - 4d0 * m2 * s)
      t1z1 = (-u * t * (t + s) + 2d0 * m2 * vgen**2) /
     .     ((u - vgen)**2 - 4d0 * m2 * u)
      t1z2 = (-s * vgen * (t + s) - 2d0 * m2 *
     .     (2d0 * t * u + (u - 2d0 * (s + vgen)) * vgen)) /
     .     ((u - vgen)**2 - 4d0 * m2 * u)
      t1p(1) = t1min
      t1p(2) = t1z
      t1p(3) = min(t1z1, t1z2)
      ...
      do i = 1, 4
         do it1 = 1, nt1
            ...
            call zd(t, tt1b, vgen)
            sib = fsir(t, tt1b, vgen, 0d0, 0d0, nn, 1, 0d0)
            idx = (i - 1) * nt1 + it1
            if (idx .eq. 1) then
               distsit1(idx) = 0d0
            else
               distsit1(idx) = distsit1(idx - 1) +
     .              (sib + sia) * (tt1b - tt1a) / 2d0
      ...
      sigrand = urand() * sig
      do it1 = 2, 4 * nt1
         if (distsit1(it1) .gt. sigrand) then
            t1gen = distart1(it1 - 1) +
     .           (distart1(it1) - distart1(it1 - 1)) *
     .           (sigrand - distsit1(it1 - 1)) /
     .           (distsit1(it1) - distsit1(it1 - 1))
      ...
      do iz = 1, nz
         ...
         sib = fsir(t, t1gen, vgen, zzb, 0d0, nn, 0, 0d0)
         if (iz .eq. 1) then
            distsiz(iz) = 0d0
         else
     ...
     .           (sib + sia) * (zzb - zza) / 2d0
      ...
      sigrand = urand() * sig
      do iz = 2, nz
         if (distsiz(iz) .gt. sigrand) then
            zgen = distarz(iz - 1) + (distarz(iz) - distarz(iz - 1)) *
     .           (sigrand - distsiz(iz - 1)) /
     .           (distsiz(iz) - distsiz(iz - 1))
            return
        endif
      enddo
...
!===============================================================================
      real(C_DOUBLE) function merad_sigr(tin, t1in, vin, zin, plin, ikeyin)
! retrun the "hard" Bremsstrahlung differential cross section
     &bind(C, name = "merad_sigr")
      use, intrinsic :: ISO_C_BINDING
!-------------------------------------------------------------------------------
      implicit none
      real(C_DOUBLE), intent(IN), VALUE :: tin, t1in, vin, zin, plin
      integer(C_INT), intent(IN), VALUE :: ikeyin
      real*8 t, t1, v, z, pl, fsir
      integer nn, ikey
      ...
      if (ikey .eq. 0) then
         call zd(t, t1, v)
         merad_sigr = fsir(t, t1, v, z, pl, nn, 0, 0d0)
      elseif (ikey .eq. 1) then
         call zd(t, t1, v)
         merad_sigr = fsir(t, t1, v, 0d0, pl, nn, 1, 0d0)
      elseif (ikey .eq. 2) then
         merad_sigr = fsir(t, 0d0, v, 0d0, pl, nn, 2, 0d0)
      endif
...
! pl: degree of polarization
      ...
      pl3 = -(ss**4 + 4d0 * m2 * (ss**2 * t + m2 *
     .     (-ss**2 + 2d0 * t**2 - 4d0 * m2 * t))) / als
      sigborn = coeb * (u1 / t**2 + u2 / u**2 + u3 / u / t +
